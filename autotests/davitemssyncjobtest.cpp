...
void DavItemsSyncJobTest::calDavSyncMalformedMissingMultiStatus()
{
    const auto [oldSyncToken, newSyncToken] = defaultOldNewSyncToken();
    const auto scenario = defaultSyncCollectionRequest(oldSyncToken) << "S: HTTP/1.1 207 Multi-Status"
                                                                     << "D: <?xml version=\"1.0\" encoding=\"utf-8\" ?>"
                                                                     << "D: <D:error xmlns:D=\"DAV:\">"
                                                                     << "D: </D:error>";
    FakeServer fakeServer;
    fakeServer.addScenario(scenario);
    fakeServer.startAndWait();
