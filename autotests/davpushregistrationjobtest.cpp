...
void DavPushRegistrationJobTest::createCalDavPushRegistration()
{
    ...
    auto registration = KDAV::DavPushRegistration();
    registration.setPushEndpoint(pushEndpoint);
    registration.setSubscriptionPublicKey(subscriptionPublicKey);
    registration.setAuthToken(authToken);
    registration.setExpiration(expiration);
    const auto job = new KDAV::DavPushRegistrationJob(davUrl, registration);
